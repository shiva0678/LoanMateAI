...
function extractWithFallback(message) {
  ...
  applicant.existingEMI = firstMatch(message, [
    /(?:existing|current|already pay|monthly)\s*(?:loan\s*)?emi\D{0,15}(?:₹|rs\.?\s*)?([\d,]+(?:\.\d+)?)\s*(k)?/i
  ], amountFromMatch);
  applicant.loanAmount = firstMatch(message, [
    /(?:need|want|loan amount|borrow)\D{0,20}(?:₹|rs\.?\s*)?([\d,]+(?:\.\d+)?)\s*(lakh|lac|k)?/i
  ], amountFromMatch);
  applicant.tenureMonths = lastMatch(message, /(?:for|tenure)\D{0,10}(\d+(?:\.\d+)?)\s*years?/gi,
    years => Math.round(Number(years) * 12));
  if (applicant.existingEMI === null && /no\s+(?:existing\s+)?emi/i.test(message)) applicant.existingEMI = 0;
  return applicant;
}
